...
class EntornoRobot(gym.Env):
    def __init__(self, num_obstacles=10):
        ...
        self.action_space = spaces.Box(
            low=np.array([0.0, -self.max_w, self.min_dt]),
            high=np.array([self.max_v, self.max_w, self.max_dt]),
            dtype=np.float32
        )
        ...
        self.observation_space = spaces.Box(
            low=np.array([0.0, -np.pi] + [0.0] * self.num_lasers + [self.min_dt, 0.0, -self.max_w]),
            high=np.array([max_dist, np.pi] + [self.laser.max_z] * self.num_lasers + [self.max_dt, self.max_v, self.max_w]),
            dtype=np.float32
        )
        ...
        self.max_episode_time = 100.0  # segundos simulados
    ...
    def _obtener_observacion(self):
        ...
        obs = [dist, angle] + laser_distances + [self.last_dt, v_real, w_real]
    ...
    def step(self, action):
        ...
        self.simulate_robot(v, w, duration=tiempo)
        ...
        self.elapsed_episode_time += tiempo
        
        self.dt_list.append(tiempo)

        # Terminar el episodio si se alcanza el límite de pasos
        timeout = self.elapsed_episode_time >= self.max_episode_time
        terminated = choque or llegada
        truncated = timeout and not terminated
        #truncated = (self.step_count >= max_steps) or (self.elapsed_episode_time >= self.max_episode_time) 
        ...
        if terminated or truncated:
            info["mean_dt"] = float(np.mean(self.dt_list))
            info["std_dt"]  = float(np.std(self.dt_list))
